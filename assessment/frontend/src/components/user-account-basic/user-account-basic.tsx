...
export const UserAccountBasic = ({ data }: { data: UserAccountBasicDataProps }) => {
  ...
  const { users, userType, isLoading, isError, error, onDelete } = data;
  ...
  const onSave = async () => {
    try {
      ...
      if (menuAction === 'DELETE_STUDENT' && onDelete) {
        onDelete(userId);
        toggleModal();
        return;
      }
  ...
  // Add delete action for students only
  if (userType === 'student' && onDelete) {
    menuActions.push({
